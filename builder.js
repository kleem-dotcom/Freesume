...
const accountGuestView=document.getElementById("accountGuestView"), accountUserView=document.getElementById("accountUserView"), accountUserName=document.getElementById("accountUserName"), accountUserEmail=document.getElementById("accountUserEmail"), accountAvatar=document.getElementById("accountAvatar"), accountHistory=document.getElementById("accountHistory");
...
function refreshAccountUI(){
  ...
  if(loggedIn){
    ...
    accountGuestView.style.display="block";
    accountUserView.style.display="none";
    accountHistory.style.display="none";
    accountHistory.innerHTML="";
  ...
  try{
    ...
    if(currentUser){
      ...
      if(rr.ok){
        ...
        if(x.resume&&x.resume.data){
          const d=x.resume.data;
          data=d.data||data;experience=d.experience||experience;education=d.education||education;projects=d.projects||projects;activeTemplate=d.template||activeTemplate;
          localStorage.setItem("freesumeResume",JSON.stringify(data));localStorage.setItem("freesumeExperience",JSON.stringify(experience));localStorage.setItem("freesumeEducation",JSON.stringify(education));localStorage.setItem("freesumeProjects",JSON.stringify(projects));localStorage.setItem("freesumeTemplate",activeTemplate);
          bindTopFields();renderLists();render();renderPhoto();applyTemplate();
        }
      }
    }
  }catch{currentUser=null;refreshAccountUI();}
}
function openAccount(){refreshAccountUI();accountModal.classList.add("open");accountModal.setAttribute("aria-hidden","false");}
...
document.getElementById("loginTab").onclick=()=>setAccountMode("login");
document.getElementById("registerTab").onclick=()=>setAccountMode("register");
document.getElementById("accountResumeBtn").onclick=()=>{closeAccount();document.getElementById("personal").scrollIntoView({behavior:"smooth",block:"start"});};
document.getElementById("accountTemplatesBtn").onclick=()=>{closeAccount();document.querySelector(".template-selector")?.scrollIntoView({behavior:"smooth",block:"center"});};
document.getElementById("accountHistoryBtn").onclick=async()=>{
  if(!currentUser)return;
  accountHistory.style.display="block";
  accountHistory.innerHTML='<div class="account-history-title">DOWNLOAD HISTORY</div><div class="account-empty">Loading…</div>';
  try{
    const r=await fetch("/api/download-history");
    const j=await r.json();
    if(!r.ok)throw Error(j.error||"Could not load download history.");
    const rows=Array.isArray(j.downloads)?j.downloads:[];
    accountHistory.innerHTML='<div class="account-history-title">DOWNLOAD HISTORY</div>'+(rows.length?rows.map(x=>`<div class="account-history-row"><span>${String(x.template||"Resume").replace(/</g,"&lt;")}</span><span>${x.at?new Date(x.at).toLocaleDateString():""}</span></div>`).join(""): '<div class="account-empty">No PDF downloads yet.</div>');
  }catch(e){accountHistory.innerHTML=`<div class="account-history-title">DOWNLOAD HISTORY</div><div class="account-empty">${e.message||"Could not load history."}</div>`;}
};
accountSubmit.onclick=async()=>{
  ...
  try{
    const body=accountMode==="login"?{email:loginEmail.value.trim(),password:loginPassword.value}:{name:registerName.value.trim(),email:registerEmail.value.trim(),password:registerPassword.value};
    ...
    }else showToast(currentUser?.name?`Welcome back, ${currentUser.name}.`:"Signed in successfully.","success");
    await syncResume();
...
logoutBtn.onclick=async()=>{try{await fetch("/api/auth/logout",{method:"POST"});}finally{currentUser=null;refreshAccountUI();closeAccount();showToast("Signed out successfully.","success");}};
loadAccount();
bindTopFields();renderLists();render();renderPhoto();applyTemplate();
