...
function refreshAccountUI(){if(currentUser){accountPill.textContent=currentUser.name;accountBtn.textContent="Account";}else{accountPill.textContent="Guest";accountBtn.textContent="Sign in";}document.getElementById("logoutBtn").style.display=currentUser?"block":"none";}
...
accountSubmit.onclick=async()=>{try{const body=accountMode==="login"?{email:loginEmail.value,password:loginPassword.value}:{name:registerName.value,email:registerEmail.value,password:registerPassword.value};const r=await fetch(`/api/auth/${accountMode}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});const j=await r.json();if(!r.ok)throw Error(j.error||"Account request failed");currentUser=j.user;refreshAccountUI();accountModal.classList.remove("open");showToast(currentUser.name?`Welcome, ${currentUser.name}.`:"Signed in");await syncResume();}catch(e){showToast(e.message);}};
logoutBtn.onclick=async()=>{await fetch("/api/auth/logout",{method:"POST"});currentUser=null;refreshAccountUI();accountModal.classList.remove("open");showToast("Signed out");};
loadAccount();
